#!/usr/bin/env python3
"""Build population-adjusted views of a frozen popularity-first top-N."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if (
        ...
        raise ValueError(
            f"Population-adjusted experiment requires {args.top_n} bands"
        )
    if origins["band_count"].sum() != args.top_n:
        raise ValueError(
            f"Raw origin concentration must cover all {args.top_n} bands"
        )
    ...
    report = {
        ...
        "formulas": {
            "selected_bands_per_million_residents": (
                f"selected top-{args.top_n} band count / FUA population "
                "* 1,000,000"
            ),
            "selected_monthly_listeners_per_resident": (
                "sum of captured monthly listeners for selected "
                f"top-{args.top_n} bands / FUA population"
            ),
        },
        "strict_mapping": {
            **strict_coverage,
        ...
        "interpretation_guardrail": (
            ...
            f"popularity-selected top {args.top_n}. They do not estimate "
            "scene depth, "
