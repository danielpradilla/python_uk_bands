...
fixed panel, catalogue rules, data, formulas and assumptions before showing
the band-level composition and unnormalized city totals. It then presents the
all-ten population-normalized result as the primary ranking and removes each
city's largest band as one separate scene-depth test. The final rank chart
compares raw totals, normalized totals and scene depth. The earlier five-band
comparison and the Google Trends, automated-MusicBrainz, symmetric-trim and
50-band work remain preserved as experiments, appendices or history rather
than competing headline methods.
