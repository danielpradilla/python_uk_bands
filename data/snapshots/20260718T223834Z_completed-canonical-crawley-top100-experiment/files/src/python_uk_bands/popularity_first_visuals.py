...
def plot_top_selected_bands(
    ...
    *,
    ...
    return _finish_chart(
        ax,
        number=number,
        title="The most-listened-to bands in the selected UK top 100",
        subtitle=(
...
def plot_population_adjusted_fua_reach(
    ...
    *,
    ...
    plot_data = strict.sort_values("top100_monthly_listeners_per_resident")
    colors = []
    hatches = []
    for row in plot_data.itertuples(index=False):
        if row.band_count == 1:
    ...
    bars = ax.barh(
        plot_data["study_city_label"],
        plot_data["top100_monthly_listeners_per_resident"],
        color=colors,
        edgecolor=HOUSE["ink"],
        linewidth=0.5,
    )
    ...
    maximum = plot_data["top100_monthly_listeners_per_resident"].max()
    ax.set_xlim(0, maximum * 1.2)
    ...
    for bar, (_, row) in zip(bars, plot_data.iterrows()):
        ax.text(
            row["top100_monthly_listeners_per_resident"] + maximum * 0.012,
            bar.get_y() + bar.get_height() / 2,
            f"{row['top100_monthly_listeners_per_resident']:.1f}x · "
            f"n={int(row['band_count'])}",
            va="center",
            color=HOUSE["ink_soft"],
            fontfamily="monospace",
    ...
    return _finish_chart(
        ax,
        number=number,
        title="Captured top-100 reach relative to FUA population",
        subtitle=(
...
def plot_multiband_stability(
    ...
    *,
    ...
    plot_data = stable.sort_values("top100_monthly_listeners_per_resident")
    colors = [
        HOUSE["blue"] if rank <= 3 else HOUSE["gray_blue"]
    ...
    for bar, (_, row) in zip(bars, plot_data.iterrows()):
        ax.text(
            row["top100_monthly_listeners_per_resident"] + maximum * 0.016,
            bar.get_y() + bar.get_height() / 2,
            f"{row['top100_monthly_listeners_per_resident']:.1f}x · "
            f"n={int(row['band_count'])}",
            va="center",
    ...
    return _finish_chart(
        ...
        subtitle=(
            ...
            f"not a replacement top-100 ranking · Spotify snapshot {snapshot_date}"
        ),
        filename=filename,
        output_dir=output_dir,
    )
...
def plot_raw_vs_normalized_fua_ranks(
    ...
    *,
    ...
    return _finish_chart(
        ...
        subtitle=(
            "Same 20 strictly mapped FUAs in both views · one-band labels include "
            f"n=1 · Spotify snapshot {snapshot_date}"
        ),
        filename=filename,
        output_dir=output_dir,
    )
