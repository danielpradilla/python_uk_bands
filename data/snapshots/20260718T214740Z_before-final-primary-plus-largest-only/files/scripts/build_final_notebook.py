...
With all ten bands included, {facts['all_ten_top']} lead. The largest selected
band supplies {facts['liverpool_concentration']:.0%} of Liverpool's reach and
...
{facts['top_excluded_top']}. A symmetric trim of the highest and lowest band
produces the same complete ranking as the largest-band-excluded test.

The most defensible reading is therefore not a single winner. Manchester and
Sheffield show comparatively broad reach across the selected catalogue, while
Liverpool's position is more dependent on the Beatles.
...
total merely because more acts were selected. The analysis then proceeds in
three stages:

| Stage | Calculation | Question answered |
|---|---|---|
| All ten | Sum all ten bands, then divide by population | How large is the selected catalogue's current reach relative to area size? |
| Largest excluded | Remove the largest band, sum the remaining nine, then divide by population | Does the result survive without its dominant act? |
| Symmetric trim | Remove the highest and lowest band, average the middle eight, then divide by population | Is the ranking robust to both tails of the selected distribution? |

The all-ten measure is the descriptive result. The largest-band-excluded
measure is the principal scene-depth sensitivity test. The symmetric trim is a
...
- **Symmetric trim:** \\(\\operatorname{{mean}}(L_{{ic}}\\text{{ after removing
  min and max}}) / P_c\\)

The symmetric estimator trims 10% from each tail—20% of the ten observations
overall. Because every area retains exactly eight bands, ranking the normalized
middle-eight mean is identical to ranking the normalized middle-eight total.
...
With all ten selected bands included, {facts['all_ten_top']} lead.
...
- **All ten bands:** {facts['all_ten_top']} have the greatest selected reach
...
- **Symmetric trim:** the complete area order is identical to the
  largest-band-excluded order. Removing the lowest selected band changes the
  scale but adds no rank information in this snapshot.
- **Near tie:** London and Birmingham's population-normalized trimmed means
  differ by only {facts['london_birmingham_gap']:.2%}; their order should not
  be treated as a meaningful separation.

The descriptive and sensitivity results answer different questions. Liverpool
has substantial current reach across the selected catalogue, but its relative
position is more dependent on one band. Manchester and Sheffield remain strong
when the dominant act is removed, which is the clearest evidence of scene depth
...
- The catalogue is manually curated, excludes solo artists, reflects the
  project's genre lane, and is not a random sample. Trimming cannot remove this
  selection bias.
- {facts['review_rows']} origin assignments are medium confidence and remain
...
- The largest-band and symmetric-trim tests describe sensitivity within ten
  selected bands. A different credible catalogue could produce a different
  order.

The supported conclusion is deliberately narrow: **within this selected
catalogue, Manchester and Sheffield retain comparatively strong
population-normalized Spotify reach beyond their largest act, while
Liverpool's rank is more superstar-dependent.** This is not a definitive
ranking of British music scenes or musical importance.
