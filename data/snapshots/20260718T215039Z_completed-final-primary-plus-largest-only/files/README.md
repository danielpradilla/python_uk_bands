...
This project asks a deliberately narrow question: which UK built-up areas have produced bands with an unusually large global Spotify footprint relative to local population?

The current result is exploratory. It uses a balanced, manually curated
100-band catalogue—ten bands for each of ten cities—and a frozen Spotify
snapshot from 17 July 2026. “Reach” means current global Spotify
...
- [Top-20 city-first experiment](notebooks/experiments/snapshots/uk_bands_top20_city_first_20260718T204000Z.ipynb)
- [Top-100 popularity-first origin experiment](notebooks/experiments/snapshots/uk_bands_top100_popularity_first_20260718T204522Z.ipynb)
- [Population-adjusted top-100 companion](notebooks/experiments/snapshots/uk_bands_top100_popularity_first_population_adjusted_20260718T204522Z.ipynb)
...
the current published analysis. The city-first notebook applies conclusions
only to the twenty largest UK urban areas; the popularity-first notebooks ask
separate geographic-concentration and population-adjusted-output questions.

## Current finding

Across all ten selected bands per city, Sheffield has the greatest current
global Spotify reach relative to built-up-area population; Manchester and
Liverpool follow. This is the primary result. As an additional scene-depth
test, Manchester ranks first after each city's largest selected act is removed,
Sheffield remains second without Arctic Monkeys and Liverpool falls to fifth
without the Beatles. The analysis status is **share with caveats** because the
catalogue is curated, genre-influenced and not a census of British music.
...
Execution reads only the selected frozen local snapshot and rewrites five PNGs
...
The canonical 100-band notebook can use an exact scene-depth timestamp, a UTC
date or `latest`. The executed notebook records the exact resolved snapshot:

```bash
# Canonical 100-band analysis
python scripts/build_final_notebook.py --snapshot 20260717T225650Z
...
- `reference/scene_depth_bands.csv`: the balanced 100-band catalogue
- `reference/built_up_areas.csv`: 2021 built-up-area population denominators
- `reference/uk_fua_top20_2021.csv`: official OECD/EU top-20 UK FUA universe
- `reference/popularity_first_overrides_20260718.csv`: reviewed identity and origin exceptions for the popularity-first experiment
- `reference/popularity_first_origin_fua_mapping_20260718.csv`: audited strict, sensitivity and excluded origin-to-FUA decisions
...
- `data/processed/popularity_first_top100_*`: frozen popularity-first selections, origin summaries and population-adjusted outputs
