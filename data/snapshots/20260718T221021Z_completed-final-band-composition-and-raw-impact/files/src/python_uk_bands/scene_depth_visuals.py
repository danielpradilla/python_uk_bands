...
def plot_raw_normalized_scene_depth_rank_comparison(
    ...
    *,
    ...
    return _finish_chart(
        ax,
        number=number,
        title="City rank across raw, normalized and scene-depth views",
        subtitle=(
            "Same ten-band catalogue · population denominator in blue and gold · "
            f"Spotify snapshot {snapshot_date}"
        ),
        filename=filename,
        output_dir=output_dir,
...
def plot_ten_band_city_stack(
    ...
    *,
    ...
    """Plot every city total as ten ranked band-level segments."""
    ...
    figure = plt.figure(figsize=(16, 10.2), layout="constrained")
    ...
    for y_position, city in enumerate(cities):
        ...
        for position, row in city_bands.iterrows():
            ...
            if value >= city_totals.max() * 0.012:
                ax.text(
                    left + value / 2,
                    y_position,
                    str(position + 1),
    ...
    return _finish_chart(
        ax,
        number=number,
        title="City monthly listeners stacked by selected band",
        subtitle=(
            "All 100 selected bands · segment numbers match the two-line key · "
            f"Spotify snapshot {snapshot_date}"
        ),
        filename=filename,
        output_dir=output_dir,
...
def plot_raw_city_totals(
    ...
    *,
    ...
    return _finish_chart(
        ...
        subtitle=(
            "Raw sum across all ten selected bands per city · "
            f"Spotify snapshot {snapshot_date}"
        ),
        filename=filename,
        output_dir=output_dir,
